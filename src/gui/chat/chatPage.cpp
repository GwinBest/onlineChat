...
namespace Gui
{
            ...
            clientInstance.value().get().RegisterReceiveMessageCallback(
                [this](const MessageBuffer::MessageNode& message) {
                    QMetaObject::invokeMethod(
                        this,
                        [this, message] { RenderLastMessage(message); },
                        Qt::QueuedConnection);
                });
        }
    }
            ...
            if (currentDateTime.date() != previousDateTime.date())
            {
                ...
                QMetaObject::invokeMethod(
                    ...
                    [this, currentSendTime] {
                        _messagesContainerLayout->addWidget(
                            ScrollArea::CreateDateDivider(currentSendTime));
                    },
                    Qt::QueuedConnection);
            }
    ...
    {
        if (const int count = layout->count(); count > 0)
        {
    ...
    QString ChatPage::ExtractDateTimeFromMessageWidget(const QWidget* messageWidget)
    {
        ...
        const QRegularExpression regexTime("<div style='color: #[0-9A-Fa-f]{6};font-size: 10px; "
                                           "text-align: right;'>(\\d{2}:\\d{2})</div>");
        const QRegularExpression regexDate("<div style='font-size: 14px;"
    ...
    void ChatPage::RenderLastMessage(const MessageBuffer::MessageNode& message) const
    {
            ...
            messageContainerSize > 0)
        {
            ...
            const auto lastRenderedMessageSendDate = ExtractDateTimeFromMessageWidget(
                lastRenderedMessage);
