...
namespace ServerNetworking
{
    ...
    bool Server::Start()
    {
        ...
        if (bind(_serverSocket, std::bit_cast<SOCKADDR*>(&_socketAddress), sizeof(_socketAddress))
            != 0)
        {
    ...
    void Server::Run()
    {
        ...
        while (true)
        {
            ...
            if (std::array<char, INET_ADDRSTRLEN> clientIp = {};
                inet_ntop(AF_INET, &_socketAddress.sin_addr, clientIp.data(), INET_ADDRSTRLEN)
                == nullptr)
            {
            ...
            if (auto it = std::ranges::find(_connections.begin(),
                                            _connections.end(),
                                            INVALID_SOCKET);
                it != _connections.end())
            {
    ...
    void Server::ClientHandler(const SOCKET clientSocket)
    {
        ...
        while (true)
        {
            if (const int32_t recvReturnValue = recv(clientSocket,
                                                     reinterpret_cast<char*>(&actionType),
                                                     sizeof(actionType),
                                                     0);
                recvReturnValue <= 0)
            {
                if (const auto it = std::find(_connections.begin(),
                                              _connections.end(),
                                              clientSocket);
                    it != _connections.end())
                {
    ...
    void Server::HandleAction(const SOCKET clientSocket, const NetworkCore::ActionType actionType)
    {
        ...
        case kCreateNewPersonalChat:        HandleCreateNewPersonalChat(clientSocket); break;
        case kSendChatMessage:              HandleSendChatMessage(clientSocket); break;
        case kAddUserCredentialsToDatabase: HandleAddUserCredentialsToDatabase(clientSocket); break;
        case kCheckUserExistence:           HandleCheckUserExistence(clientSocket); break;
        case kCheckIsUserDataFromFileValid: HandleCheckIsUserDataFromFileValid(clientSocket); break;
        case kGetUserNameFromDatabase:      HandleGetUserNameFromDatabase(clientSocket); break;
        case kGetUserIdFromDatabase:        HandleGetUserIdFromDatabase(clientSocket); break;
        case kFindMatchingChats:            HandleFindMatchingChats(clientSocket); break;
        case kGetAvailableChatsForUser:     HandleGetAvailableChatsForUser(clientSocket); break;
        case kReceiveAllMessagesForSelectedChat:
            HandleReceiveAllMessagesForSelectedChat(clientSocket);
            break;
        default: std::cerr << "Undefined action type\n"; break;
        }
    }
    ...
    void Server::HandleCreateNewPersonalChat(const SOCKET clientSocket)
    {
        ...
        recv(clientSocket,
             reinterpret_cast<char*>(&receiverUserNameSize),
             sizeof(receiverUserNameSize),
             NULL);
        ...
        try
        {
            ...
            auto query = std::format("INSERT INTO chats (name, photo, created_at) "
                ...
                resultSet->next())
            {
                ...
                query = std::format("INSERT INTO users_chats (user_id, chat_id, joined_at) "
                                    "SELECT {}, {}, NOW() "
                                    "WHERE NOT EXISTS ("
                                    "    SELECT 1 FROM users_chats WHERE user_id = {} AND "
                                    "chat_id = {}"
                                    ");",
                                    senderUserId,
                                    chatId,
                ...
                query = std::format("INSERT INTO users_chats (user_id, chat_id, joined_at) "
                                    "SELECT (SELECT id FROM users WHERE name = '{}' LIMIT 1), "
                                    "{}, NOW() "
                                    "WHERE NOT EXISTS ("
            ...
            send(clientSocket,
                 reinterpret_cast<const char*>(&actionType),
                 sizeof(actionType),
                 NULL);
        ...
        catch (const sql::SQLException& e)
        {
            ...
            SendServerErrorMessage(clientSocket,
                                   "Server error: server cant create new personal chat");
    ...
    void Server::HandleSendChatMessage(const SOCKET clientSocket)
    {
        ...
        try
        {
            ...
            query = std::format("INSERT INTO messages (chat_id, user_id, content, sent_at) "
                                "VALUES ("
                                "    (SELECT id FROM chats WHERE id = {} LIMIT 1), "
                                "    {}, '{}', NOW()"
                                ");",
            ...
            query = std::format("SELECT users_chats.user_id, messages.sent_at "
                                "FROM users_chats "
        ...
        catch (const sql::SQLException& e)
        {
            ...
            SendServerErrorMessage(clientSocket,
                                   "Server error: server cant push message to database");
    ...
    void Server::HandleAddUserCredentialsToDatabase(const SOCKET clientSocket)
    {
        ...
        try
        {
            auto query = std::format("INSERT INTO users(name, login, password) "
                                     "VALUES('{}', '{}', {});",
        ...
        catch (const sql::SQLException& e)
        {
            ...
            SendServerErrorMessage(clientSocket,
                                   "Server error: server cant add user credentials to database");
    ...
    void Server::HandleCheckUserExistence(const SOCKET clientSocket)
    {
        ...
        try
        {
            auto query = std::format("SELECT * FROM users "
    ...
    void Server::HandleCheckIsUserDataFromFileValid(const SOCKET clientSocket)
    {
        ...
        catch (const sql::SQLException& e)
        {
            ...
            SendServerErrorMessage(clientSocket,
                                   "Server error: server cant check is data from file valid");
        }
    }
    ...
    void Server::HandleGetUserNameFromDatabase(const SOCKET clientSocket)
    {
        ...
        try
        {
            auto query = std::format("SELECT name FROM users "
                                     "WHERE login = '{}';",
                                     userPacket.login.c_str());
            auto* resultSet = Database::DatabaseHelper::GetInstance().ExecuteQuery(query);
        ...
        catch (const sql::SQLException& e)
        {
            ...
            SendServerErrorMessage(clientSocket,
                                   "Server error: server cant get user name from database");
        }
    }
    ...
    void Server::HandleGetUserIdFromDatabase(const SOCKET clientSocket)
    {
        ...
        try
        {
            auto query = std::format("SELECT id FROM users "
                                     "WHERE login = '{}';",
                                     userPacket.login.c_str());
            auto* resultSet = Database::DatabaseHelper::GetInstance().ExecuteQuery(query);
    ...
    void Server::HandleFindMatchingChats(const SOCKET clientSocket)
    {
        try
        {
            ...
            auto query = std::format("(SELECT DISTINCT "
                                     "c.id AS chat_id, "
                                     ...
                                     "LEFT JOIN "
                                     "users_chats uc2 ON c.id = uc2.chat_id AND uc2.user_id != "
                                     "u.id "
                                     "LEFT JOIN "
            ...
            {
                send(clientSocket,
                     reinterpret_cast<char*>(&chatIdResult[i]),
                     sizeof(chatIdResult[0]),
                     NULL);

                size_t resultLength = chatNameResult[i].size();
                send(clientSocket,
                     reinterpret_cast<char*>(&resultLength),
                     sizeof(resultLength),
                     NULL);
    ...
    void Server::HandleGetAvailableChatsForUser(const SOCKET clientSocket)
    {
        try
        {
            ...
            auto query = std::format("SELECT DISTINCT "
                                     ...
                                     "LEFT JOIN users_chats uc2 ON c.id = uc2.chat_id "
                                     "AND uc2.user_id != u.id "
        ...
        catch (const sql::SQLException& e)
        {
            ...
            SendServerErrorMessage(clientSocket,
                                   "Server error: server cant get available chats for user");
        }
    }
    ...
    void Server::HandleReceiveAllMessagesForSelectedChat(const SOCKET clientSocket)
    {
        ...
        try
        {
            ...
            auto messageTypeResult = std::make_unique<MessageBuffer::MessageStatus[]>(
                resultSet->rowsCount());
            ...
            for (size_t i = 0; i < count; ++i)
            {
                ...
                send(clientSocket,
                     reinterpret_cast<char*>(&messageTypeResult[i]),
                     sizeof(messageTypeResult[i]),
                     NULL);

                size_t sendTimeSize = messagesSendTime[i].size();
                send(clientSocket,
                     reinterpret_cast<char*>(&sendTimeSize),
                     sizeof(sendTimeSize),
                     NULL);
        ...
        catch (const sql::SQLException& e)
        {
            ...
            SendServerErrorMessage(clientSocket,
                                   "Server error: server cant receive all "
    ...
    NetworkCore::UserPacket Server::ReceiveUserCredentialsPacket(const SOCKET clientSocket)
    {
        ...
        recv(clientSocket,
             reinterpret_cast<char*>(&userLoginLength),
             sizeof(userLoginLength),
             NULL);
    ...
    void Server::SendServerErrorMessage(const SOCKET clientSocket,
                                        const std::string_view errorMessage) noexcept
    {
        ...
        send(clientSocket,
             reinterpret_cast<const char*>(&errorMessageSize),
             sizeof(errorMessageSize),
             NULL);
