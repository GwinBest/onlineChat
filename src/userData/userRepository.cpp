...
namespace UserData
{
    Awaitable<std::optional<bool>> UserRepository::PushUserCredentialsToDatabaseAsync(const User user)
    {
    ...
    Awaitable<std::optional<std::string>> UserRepository::GetUserNameFromDatabaseAsync(const std::string userLogin)
    {
    ...
    Awaitable<std::optional<size_t>> UserRepository::GetUserIdFromDatabaseAsync(const std::string userLogin)
    {
    ...
    Awaitable<std::optional<std::vector<ChatSystem::ChatInfo>>> UserRepository::FindMatchingChatsAsync(const size_t currentUserId,
                                                                                                       std::string pattern)
    {
        auto future = std::async(std::launch::async, [currentUserId, pattern]() -> std::optional<std::vector<ChatSystem::ChatInfo>>
                                 {
            const NetworkCore::UserPacket request =
                {
    ...
    Awaitable<std::optional<std::vector<ChatSystem::ChatInfo>>> UserRepository::GetAvailableChatsForUserAsync(const size_t userId)
    {
        auto future = std::async(std::launch::async, [userId]() -> std::optional<std::vector<ChatSystem::ChatInfo>>
                                 {
        const NetworkCore::UserPacket request =
            {
    ...
    Awaitable<std::optional<std::vector<MessageBuffer::MessageNode>>> UserRepository::GetAvailableChatMessagesAsync(const size_t userId,
                                                                                                                    const size_t chatId)
    {
        auto future = std::async(std::launch::async, [userId, chatId]() -> std::optional<std::vector<MessageBuffer::MessageNode>>
                                 {
    ...
    Awaitable<std::optional<size_t>> UserRepository::CreateNewPersonalChatAsync(const size_t senderUserId,
                                                                                const std::string receiverUserName)
    {
        auto future = std::async(std::launch::async, [senderUserId, &receiverUserName]() -> std::optional<size_t>
                                 {
        const std::optional<std::reference_wrapper<ClientNetworking::Client>> instance = ClientNetworking::Client::GetInstance();
        if (!instance.has_value())
            return std::nullopt;
